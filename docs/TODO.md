...
| **Completion** | 正则 + 工作区索引 + 命名导入导出，已加入 `build()` 内 UI 组件上下文补全，并开始复用组件树分析结果 | AST 感知、上下文敏感补全 |
...
- [ ] **`build()` 方法分析** —— 已能提取 UI 组件调用、嵌套组件树，并给组件树节点附加源码 range；但尚未构建完整组件语义模型（如 props/参数/路径等）
